...
def filter_empty_cell_lines(sensitivities, info_cell_lines):
    ...
    return numpy.array(filtered_sensitivities), filtered_info_cell_lines            
...
''' Cap high values. '''
cap = 100
sensitivities_filtered = cap_high_values(sensitivities_filtered, cap)

''' Make the names into numpy string arrays so we can store them easily. '''
...
plt.figure()
plt.hist([v for v in sensitivities_filtered.flatten() if not numpy.isnan(v)], bins=range(0,cap+1,1))
plt.title("Distribution of CTRP EC50 values")
plt.savefig('./../plots/distribution_ctrp_ec50.pdf')