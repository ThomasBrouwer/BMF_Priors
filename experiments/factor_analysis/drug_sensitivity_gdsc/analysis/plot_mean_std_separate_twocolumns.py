...
plot_file = folder_plots+"factor_analysis_separate_twocolumns.png"
...
for i, (name, (avr_mean_U, avr_std_U, avr_mean_V, avr_std_V)) in enumerate(name_meanU_stdU_meanV_stdV):
    axes[i/2][i%2].bar(x,           avr_mean_U, bar_width, color='b')#, yerr=avr_std_U, error_kw=dict(ecolor='gray'))
    axes[i/2][i%2].bar(x+bar_width, avr_mean_V, bar_width, color='r')#, yerr=avr_std_V, error_kw=dict(ecolor='gray'))
